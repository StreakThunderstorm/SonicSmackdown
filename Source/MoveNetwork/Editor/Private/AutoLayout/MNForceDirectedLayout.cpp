#include "MNForceDirectedLayout.h"
...
UMNForceDirectedLayout::UMNForceDirectedLayout()
{
	bRandomInit = false;
	CoolDownRate = 10;
...
void UMNForceDirectedLayout::Layout(UEdGraph* _EdGraph)
{
...
FBox2D UMNForceDirectedLayout::LayoutOneTree(UMNNode* RootNode, const FBox2D& PreTreeBound)
{
	...
	for (int32 IterrationNum = 0; IterrationNum < MaxIteration; ++IterrationNum)
	{
		...
		while (CurrLevelNodes.Num() != 0)
		{
			for (int32 i = 0; i < CurrLevelNodes.Num(); ++i)
			{
				...
				UMNNode_EdNode* EdNode_ParentNode = EdGraph->NodeMap[Node];

				for (int32 j = 0; j < Node->ChildrenNodes.Num(); ++j)
				{
					NextLevelNodes.Add(Node->ChildrenNodes[j]);

					UMNNode_EdNode* EdNode_ChildNode = EdGraph->NodeMap[Node->ChildrenNodes[j]];
